...
A five-capture replay measured what this costs against the local-fallback behavior this gate replaces (accepting a repair on local proximity alone, without requiring the opposite-parity confirmation above): -601 messages net, 704,323 against a 704,924 baseline, across `sawbird_256_3`, `jim_merk_samples_256`, `rhodan_samples_6M`, `halde_6M_1` and `halde_10M_0` (`-s 2.56 -u 12 -q` for the RTL-SDR-format captures, `-s 6 -u 24 -q` / `-s 10 -u 24 -q` for the Airspy-format ones). 561 of the 601 were repaired positions; the other 40 were different message types that stopped being corroborated once those positions no longer kept the aircraft's cache/trust state fresh (see below). Of the 561: 554 were discarded for a missing or stale opposite parity, 4 for a missing clean reference, and 3 never reached this gate at all. None of the 601 were confirmed false. A later clean opposite parity, within the pairing window, was found for 238 of the 561 -- forming a global pair within 100 km of that aircraft's logged clean reference, most within a few km -- which is evidence some of these repairs were correct, not proof every one was; the remaining 323 have no such evidence either way, which is not evidence they were wrong. Do not read the 601 as that many phantom positions removed: most are positions this gate could not confirm in time, not positions it identified as false.

Rejecting a repair can also cost later, unrelated messages for the same aircraft, through a mechanism this gate does not touch: address trust. This policy makes no change to trust, and a discarded repair never renews it -- but ICAOTable's own cache-slot eviction ttl (`TTL_not_trusted`, decremented once per second, independently of and shorter than the trust ttl) still runs down on its own schedule. If nothing else reaches that aircraft for long enough, the slot expires, and the next CRC-clean frame for that address is treated exactly like a brand-new address's first sighting: withheld until a second sighting confirms it. This is an existing, general transition (ttl expiry), not something specific to a discarded repair or new to this gate. Whether to refresh a slot's ttl for a discarded-but-locally-plausible repair, and what that would risk (sustaining a spurious address on unconfirmed data), is a separate question with its own tradeoffs -- out of scope here.
