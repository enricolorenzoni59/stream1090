...
template <typename T> class InputDeviceBase {
  public:
    ...
    void writeDataToBuffer(const T* data, size_t n) {
        ...
        const bool gap = std::chrono::duration_cast<std::chrono::milliseconds>(now - m_lastAccounting).count() > 150;
        if (m_accT0.time_since_epoch().count() == 0) {
            ...
            const int64_t deficit = std::max<int64_t>(expected - (int64_t)(pairs - m_accPairs0), 0);
            m_dropDeficit.store(deficit, std::memory_order_relaxed);
            {
                ...
                while (deficit > (int64_t)prev &&
                       !m_maxDeficit.compare_exchange_weak(prev, (uint64_t)deficit, std::memory_order_relaxed)) {
                }
            }
