...
inline bool parsePositiveDecimal(const std::string& serial, uint64_t& value) noexcept {
    if (serial.empty())
        return false;
...
inline int resolveIndex(const std::string& requested, const std::vector<std::string>& available) noexcept {
    ...
    for (std::size_t i = 0; i < available.size(); ++i) {
        ...
        if (parsePositiveDecimal(available[i], availableValue) && availableValue == requestedValue)
            return static_cast<int>(i);
    }
