...
class ICAOTable {
  public:
    ...
    void noteCprClean(const Iterator& entry, bool odd, uint32_t latCpr, uint32_t lonCpr, uint64_t now,
                      uint64_t pairWindow) noexcept {
        auto& state = m_squawkAlt[entry.key];
        if (odd) {
        ...
        double lat = 0.0;
        double lon = 0.0;
        if (!ModeS::decodeCprGlobal(state.cpr_even_lat, state.cpr_even_lon, state.cpr_odd_lat, state.cpr_odd_lon, lat,
                                    lon))
            return;
    ...
    bool cachedPosition(const Iterator& entry, int32_t& latE5, int32_t& lonE5, uint64_t now,
                        uint64_t maxAge) const noexcept {
  ...
  private:
    ...
    bool confirmRejectedFrame(uint32_t icao, uint64_t high, uint64_t low, uint8_t bits) noexcept {
        const uint64_t mixed = low ^ (high * 0x9e3779b97f4a7c15ull) ^ (uint64_t(icao) * 0xbf58476d1ce4e5b9ull);
        auto& candidate = m_rejectedCandidates[mixed & (RejectedCandidateCount - 1)];
        const auto age = m_df11Clock - candidate.firstSeen;

        if (candidate.bits == bits && candidate.icao == icao && candidate.high == high && candidate.low == low) {
