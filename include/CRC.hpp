...
template <uint8_t numBits> constexpr crc_t compute(const Bits128& bits) noexcept {
    crc_t result = 0;
...
template <uint8_t bitIndex> constexpr crc_t delta() noexcept {
    crc_t crc = 0x1;
...
template <> constexpr crc_t delta<111>() noexcept {
    return 0x3935EA;
}

// precomputed value for 56th bit
template <> constexpr crc_t delta<55>() noexcept {
    return 0x18567;
}
