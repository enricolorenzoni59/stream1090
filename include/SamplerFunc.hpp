...
template <size_t RatioInput, size_t RatioOutput> constexpr auto makeLinearInterpTable() {
...
template <size_t RatioInput, size_t RatioOutput> constexpr auto makeSharpeningCubicTable() {
...
template <size_t RatioInput, size_t RatioOutput, size_t NumBlocks> struct SamplerFunc {

    static constexpr bool UseSharpeningCubic =
        STREAM1090_INTERP == 6 && ((RatioInput == 1 && RatioOutput == 5) || (RatioInput == 8 && RatioOutput == 25) ||
    ...
    static constexpr void sampleLinearBlock(const int32_t* __restrict in, int32_t* __restrict out) noexcept {
        using namespace SamplerFunc_details;
        for (size_t j = 0; j < RatioOutput; ++j) {
            const int32_t r = a[j];
            const int32_t l = (1 << WeightFracBits) - r;
            const int64_t acc = int64_t(l) * int64_t(in[k[j]]) + int64_t(r) * int64_t(in[k[j] + 1]);
            out[j] = int32_t(acc >> WeightFracBits);
        }
    }

#if STREAM1090_INTERP == 6
    static constexpr auto weights = SamplerFunc_details::makeSharpeningCubicTable<RatioInput, RatioOutput>();
#endif

    static constexpr void sample(const int32_t* __restrict in, int32_t* __restrict out) noexcept {
#if STREAM1090_INTERP == 6
            ...
            for (size_t i = 1; i + 1 < NumBlocks; ++i) {
                for (size_t j = 0; j < RatioOutput; ++j) {
                    ...
                    const int64_t acc = int64_t(weights[j][0]) * int64_t(p[-1]) +
                                        ...
                                        int64_t(weights[j][2]) * int64_t(p[1]) + int64_t(weights[j][3]) * int64_t(p[2]);
