...
template <typename Field> constexpr typename Field::Type getField(const uint64_t& bits) noexcept {
    static_assert(Field::MessageLength == 56);
    return BitUtils::getBits<typename Field::Type, Field::HI, Field::LO>(bits);
}

template <typename Field> constexpr typename Field::Type getField(const Bits128& bits) noexcept {
