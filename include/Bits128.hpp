...
class alignas(16) Bits128 {
  public:
    ...
    constexpr const uint64_t& high() const noexcept {
        return m_bits[1];
    }
    constexpr const uint64_t& low() const noexcept {
        return m_bits[0];
    }

    constexpr uint64_t& high() noexcept {
        return m_bits[1];
    }
    constexpr uint64_t& low() noexcept {
        return m_bits[0];
    }
    ...
    template <typename T> constexpr Bits128 operator&(const T& other) noexcept {
        Bits128 res(*this);
        return res &= other;
    }
    ...
    template <std::size_t numBits> constexpr void copyToBitset(std::bitset<numBits>& bits) const noexcept {
