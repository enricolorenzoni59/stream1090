 ...
 *
 * Four outputs per vector, one broadcast tap, no horizontal reduction at all.
 * On AArch64 the broadcast disappears too, because FMLA can take the tap
 * straight from a lane of the tap vector.
 *
 * Summation stays in tap order, exactly like the plain scalar loop.
 */
namespace FirDetail {

    // tap count rounded up to a full SIMD vector
...
#if STREAM1090_HAS_NEON
        for (; i + 4 <= n; i += 4) {
            float32x4_t accI = vdupq_n_f32(0.0f);
            float32x4_t accQ = vdupq_n_f32(0.0f);

            for (size_t k = 0; k < numTaps; k += 4) {
    #if defined(__aarch64__)
                // one vector holds four taps, FMLA picks the lane it needs
                const float32x4_t t = vld1q_f32(taps + k);
                accI = vfmaq_laneq_f32(accI, vld1q_f32(wI + i + k + 0), t, 0);
                accQ = vfmaq_laneq_f32(accQ, vld1q_f32(wQ + i + k + 0), t, 0);
                accI = vfmaq_laneq_f32(accI, vld1q_f32(wI + i + k + 1), t, 1);
                accQ = vfmaq_laneq_f32(accQ, vld1q_f32(wQ + i + k + 1), t, 1);
                accI = vfmaq_laneq_f32(accI, vld1q_f32(wI + i + k + 2), t, 2);
                accQ = vfmaq_laneq_f32(accQ, vld1q_f32(wQ + i + k + 2), t, 2);
                accI = vfmaq_laneq_f32(accI, vld1q_f32(wI + i + k + 3), t, 3);
                accQ = vfmaq_laneq_f32(accQ, vld1q_f32(wQ + i + k + 3), t, 3);
    #else
                for (size_t l = 0; l < 4; l++) {
                    accI = vmlaq_n_f32(accI, vld1q_f32(wI + i + k + l), taps[k + l]);
                    accQ = vmlaq_n_f32(accQ, vld1q_f32(wQ + i + k + l), taps[k + l]);
                }
    #endif
            }

            vst1q_f32(outI + i, accI);
            vst1q_f32(outQ + i, accQ);
        }
#else
