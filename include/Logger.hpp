...
enum class Level { ERROR = 0, WARN = 1, MSG = 2, INFO = 3, DEBUG = 4 };
...
inline std::string short_timestamp() {
    ...
    oss << std::put_time(std::localtime(&t), "%H:%M:%S") << "." << std::setw(3) << std::setfill('0') << ms.count();
    return oss.str();
}
...
class Logger {
  public:
    ...
    void write(Level lvl, const std::string& src, const std::string& msg) {
        ...
        std::cerr << short_timestamp() << " [" << src << "] " << msg << "\n";
    }
...
class Stream {
  public:
    Stream(Level lvl, const std::string& src) : level_(lvl), source_(src) {}
...
inline Stream error(const std::string& src) {
    return Stream(Level::ERROR, src);
}
inline Stream warn(const std::string& src) {
    return Stream(Level::WARN, src);
}
inline Stream msg(const std::string& src) {
    return Stream(Level::MSG, src);
}
inline Stream info(const std::string& src) {
    return Stream(Level::INFO, src);
}
inline Stream debug(const std::string& src) {
    return Stream(Level::DEBUG, src);
}
