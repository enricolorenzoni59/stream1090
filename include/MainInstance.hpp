...
template <typename Tuple, typename F> constexpr bool for_each_in_tuple(const Tuple& t, F&& f) {
    bool done = false;
    std::apply([&](auto const&... elems) { ((!done && f(elems) ? done = true : false), ...); }, t);
    return done;
}
...
std::optional<bool> runInstanceFromPresets(const CompileTimeVars& compileTimeVars, const RuntimeVars& runtimeVars) {
    ...
    for_each_in_tuple(presets, [&](auto const& p) {
        ...
        if (P::RawFormatType::id == compileTimeVars.rawFormat && P::inputRate == compileTimeVars.inputRate &&
            P::outputRate == compileTimeVars.outputRate && P::pipelineOption == compileTimeVars.pipelineOption) {
