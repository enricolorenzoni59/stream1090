...
template <std::size_t hi, std::size_t lo> struct BitRange {
    static_assert(lo <= hi);
    static constexpr std::size_t HI = hi;
    static constexpr std::size_t LO = lo;
};

template <typename ResType, std::size_t hi, std::size_t lo> constexpr ResType getBits(uint64_t bits) noexcept {
    static_assert(lo <= hi);
    static_assert(hi < 64);
...
template <typename ResType, std::size_t hi, std::size_t lo> constexpr ResType getBits(const Bits128& bits) noexcept {
    return getBits<ResType, hi, lo>(bits.high(), bits.low());
}

template <typename ResType, typename Range> constexpr ResType getBits(uint64_t bits) noexcept {
    return getBits<ResType, Range::HI, Range::LO>(bits);
}

template <typename ResType, typename Range> constexpr ResType getBits(uint64_t high, uint64_t low) noexcept {
    return getBits<ResType, Range::HI, Range::LO>(high, low);
}

template <typename ResType, typename Range> constexpr ResType getBits(const Bits128& bits) noexcept {
    return getBits<ResType, Range::HI, Range::LO>(bits);
}
} // end of namespace
