...
static int is_fvco_valid(uint32_t fvco_z)
{
	...
	if (fvco_z/1000 < E4K_FVCO_MIN_KHZ ||
		...
		rtlsdr_log(RTLSDR_LOG_ERROR, "[E4K] Fvco %u invalid\n", fvco_z);
		return 0;
	}
...
static int is_fosc_valid(uint32_t fosc)
{
	if (fosc < MHZ(16) || fosc > MHZ(30)) {
		rtlsdr_log(RTLSDR_LOG_ERROR, "[E4K] Fosc %u invalid\n", fosc);
		return 0;
	}
...
{
	if (z > 255) {
		rtlsdr_log(RTLSDR_LOG_ERROR, "[E4K] Z %u invalid\n", z);
		return 0;
	}
...
uint32_t e4k_compute_pll_params(struct e4k_pll_params *oscp, uint32_t fosc, uint32_t intended_flo)
{
	...
	//rtlsdr_log(RTLSDR_LOG_INFO, "[E4K] Fint=%u, R=%u\n", intended_flo, r);
...
int e4k_tune_freq(struct e4k_state *e4k, uint32_t freq)
{
	...
	if (!(rc & 0x01)) {
		rtlsdr_log(RTLSDR_LOG_WARN, "[E4K] PLL not locked for %u Hz!\n", freq);
		return -1;
	}
...
int e4k_dc_offset_gen_table(struct e4k_state *e4k)
{
	...
	for (i = 0; i < ARRAY_SIZE(dc_gain_comb); i++) {
		...
		rtlsdr_log(RTLSDR_LOG_DEBUG, "[E4K] Table %u I=%u/%u, Q=%u/%u\n",
