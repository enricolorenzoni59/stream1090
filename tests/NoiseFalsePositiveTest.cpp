...
struct CountingHandler {
    explicit CountingHandler(std::vector<EmittedFrame>& uniqueFrames) : m_uniqueFrames(uniqueFrames) {}
  ...
  private:
    void logFrame(const EmittedFrame& frame) {
        const auto seen = std::find_if(m_uniqueFrames.begin(), m_uniqueFrames.end(), [&](const auto& other) {
            return other.isLong == frame.isLong && other.high == frame.high && other.low == frame.low;
        });
...
class NoiseReader : public InputReaderBase<IQ_UINT16_RAW_AIRSPY, Sampler::InputBufferSize, NoPipeline> {
  public:
    NoiseReader(NoPipeline& pipeline, uint64_t offset, uint64_t positions)
        : InputReaderBase<IQ_UINT16_RAW_AIRSPY, Sampler::InputBufferSize, NoPipeline>(pipeline), m_offset(offset),
          m_remaining(positions) {}

    void readMagnitude(int32_t* out) noexcept {
        const size_t count = std::min<uint64_t>(m_remaining, Sampler::InputBufferSize);
...
int main() {
    ...
    for (size_t i = 0; i < NoiseCases.size(); ++i) {
        ...
        std::printf("FAILED case %zu: pure noise emitted %llu frame%s\n", i + 1,
                    static_cast<unsigned long long>(handler.frameCount), handler.frameCount == 1 ? "" : "s");
    }

    if (failedCases == 0)
        return 0;

    std::printf("FAILED: %zu of %zu pure-noise windows emitted %zu distinct frames\n", failedCases, NoiseCases.size(),
                uniqueFrames.size());
    return 1;
}
