...
int main() {
    ...
    check("zero-padded exact match", resolve("00000002", {"2", "00000002"}), 1);
    check("short exact match", resolve("2", {"00000002", "2"}), 1);
    check("hex-looking exact match", resolve("0x1A", {"26", "0x1A"}), 1);
    check("all-zero exact match", resolve("00000000", {"11111111", "00000000"}), 1);
    check("alphanumeric exact match", resolve("ADSBX-ORANGE-1090", {"00000001", "ADSBX-ORANGE-1090"}), 1);
    ...
    // lookup failed.
    check("legacy decimal fallback", resolve("2", {"00000001", "00000002"}), 1);
    check("legacy decimal eight fallback", resolve("8", {"00000008", "00000009"}), 0);
    ...
    check("zero has no numeric fallback", resolve("0", {"00000000"}), -1);
    check("partial decimal has no fallback", resolve("8suffix", {"00000008"}), -1);
    check("missing serial", resolve("missing", {"00000001", "00000002"}), -1);
